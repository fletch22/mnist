...
if RUNNING_LOCAL is True:
    from app.stacking.cnn.F22StackingCnn import F22StackingCnn
else:
    SESSION_DIR = ''
    logging.info = print
...
def get_vgg16():
    ...
    x = Dense(10, activation='softmax', name='predictions')(model_vgg16_conv.layers[-2].output)
...
def get_stacking(X_train, Y_train, X_test, Y_test, batch_size, epochs):
    ...
    # base_clfs = [f22StackingCnn, clf_gnb, clf_gbc, clf_xgb, clf_multinomial_nb, clf_complement_nb, clf_bernoulli_nb,
    #              clf_tree_dec, clf_tree_extra,
    #              clf_svm, clf_log_reg_100, clf_log_reg_10k, clf_kneighbors, clf_rand_forest_50, clf_rand_forest_10,
    #              bernoulli_rbm, mlp, adaboost]

    base_clfs = [clf_tree_extra]
...
def fit(clf_list, X, y):
    ...
    for label, clf in clf_list:
        ...
        print("Accuracy: {:0.2f} (+/- {:0.2f}) {}: elapsed: {:.2f} min".format(scores.mean(), scores.std(), label,
                                                                               elapsed))
...
def main(sample_number, epochs):
    ...
    for i in range(0, training_iter):
        clf_list, clf_stacking = get_stacking(X_train, y_train, X_test, y_test, 32, epochs)
    ...
    #     X_test = get_test()
    #     predictions = clf_stacking.predict(X_test)
    #
    #     logging.info("Results {}:".format(predictions))
    #
